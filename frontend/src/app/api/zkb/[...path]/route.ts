...
async function proxy(request: NextRequest, context: {params: {path: string[]}}) {
  ...
  if (!session) return Response.json({detail:"Admin session required"}, {status:401});
  if (!canProxy(session, request.method, context.params.path)) {
    return Response.json({detail:"Admin role does not permit this operation"}, {status:403});
  }
