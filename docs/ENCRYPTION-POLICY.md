...
For backup-key rotation: generate a replacement 256-bit key, switch future backups to it, retain every prior decryption key for as long as any archive encrypted under that key must remain recoverable, verify a new backup under the replacement key, and complete an isolated restore drill. An old key may be destroyed only after all dependent archives and legal-hold recovery points have expired, or after those exact historical archives have been content-preservingly re-encrypted and independently verified under a replacement key. Creating a new backup from current live state is not a substitute for preserving an older recovery point. Loss of required decryption material is an availability failure; disclosure requires rotation and incident handling for the affected archive scope.
...
Each retained archive must have an operator record mapping it to the key generation that can decrypt it. Retention policy must keep that key generation available for at least as long as any corresponding archive, legal hold, or recovery objective requires. Before destroying an old key, operators must either expire every dependent archive/recovery obligation or use a content-preserving re-encryption procedure that transforms the exact historical archive, verifies authenticated decryption and manifest integrity under the replacement key, and completes an isolated restore drill from that preserved recovery point. Until zknowbase provides such a documented re-encryption workflow, the safe policy is to retain the old key with independently protected escrow for the lifetime of every dependent archive. Key escrow copies should be access-controlled, periodically inventoried, and recovery-tested without exposing key material in logs, tickets, CI artifacts, or telemetry.
