...
@lru_cache(maxsize=8)
def _sqlite_ingestion_queue(db_path: str, default_tenant_id: str) -> TenantIngestionQueue:
    return TenantIngestionQueue(
        SQLiteIngestionQueue(Path(db_path)),
        default_tenant_id=default_tenant_id,
        sqlite_path=db_path,
    )
...
def ingestion_queue(settings: Settings) -> TenantIngestionQueue:
    if settings.metadata_backend == "postgres":
        assert settings.postgres_url is not None
    ...
    return _sqlite_ingestion_queue(str(settings.metadata_db), settings.default_tenant_id)
