...
- integrity-checked backup archives with tenant ownership preserved across supported restore paths and optional authenticated AES-256-GCM portable-backup encryption.

See `docs/ENCRYPTION-POLICY.md` for the at-rest/in-transit encryption boundary, key ownership, rotation, backup envelope, and explicit non-claims.
...
For portable backups containing sensitive data, configure an owner-only 32-byte backup key through `ZKB_BACKUP_ENCRYPTION_KEY_FILE` and set `ZKB_BACKUP_REQUIRE_ENCRYPTION=true` when policy requires fail-closed rejection of plaintext archives. Keep backup decryption keys separate from the archives they protect and include them in DR recovery-key procedures.
...
For a suspected credential compromise, revoke/rotate affected service keys immediately, rotate deployment/provider/OIDC secrets according to their ownership boundary, invalidate Admin sessions by rotating the session secret when necessary, and preserve relevant audit evidence. For a storage or backup-encryption key compromise, follow the relevant rekey procedure and treat affected backups/volumes as potentially disclosed according to the data classification in `docs/ENCRYPTION-POLICY.md`.

## Security acceptance

A change is not considered production-ready merely because it builds. Required security regression coverage includes authentication/authorization failures, tenant-isolation negative cases, upload active-content rejection, configured ClamAV fail-closed behavior, OIDC state/PKCE validation, scoped-key lifecycle behavior, tenant-aware backup/restore preservation, and encrypted-backup wrong-key/tamper/plaintext-policy failure cases where applicable.
